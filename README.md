...
`xless` is a Rust pager with less-style navigation, safe ANSI color pass-through, mark/jump navigation, blank-line squeezing, and syntax highlighting for common source files.
...
- Less-style navigation, counted motions, startup commands like `+42`, `+/pattern`, `+G`, and `+F`, startup search with `-p/--pattern`, incremental search over visible text, mark/jump keys, blank-line squeezing, and editor handoff.
