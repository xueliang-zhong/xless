...
- `!` opens a shell-command prompt. Enter `!git status`, `!xcat src/lib.rs`, or shell pipelines such as `!git diff --color=always | less -R`; `XLESS_FILE`, `XLESS_PATH`, `XLESS_LINE`, and `XLESS_GLOBAL_LINE` are set for the launched process.
...
- `!` uses your shell so pipelines, redirection, and expansion behave the way they do in `bash` or `zsh`.
